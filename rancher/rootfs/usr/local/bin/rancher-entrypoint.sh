#!/bin/bash
# Upstream rancher/rancher entrypoint with HAOS cgroup handling.
# Source: https://github.com/rancher/rancher/blob/main/package/entrypoint.sh
# Sync when bumping RANCHER_TAG if this file diverges from upstream.
...
# Moby dind cgroup v2 init — skipped when Supervisor mounts cgroup read-only (HAOS apps).
if [ ! -e /run/secrets/kubernetes.io/serviceaccount ] && [ -f /sys/fs/cgroup/cgroup.controllers ]; then
  if mkdir -p /sys/fs/cgroup/init 2>/dev/null; then
    ...
    echo "INFO: skipping cgroup v2 init (read-only /sys/fs/cgroup — normal on Home Assistant OS)"
  fi
fi
